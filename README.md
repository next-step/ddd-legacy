...
- [ ] 각 메뉴는 소속된 **메뉴 그룹의 ID**를 참조한다.
  ...
  #### 메뉴 숨김
    - 메뉴를 **비노출 상태로 변경**할 때는 메뉴 노출 때와 달리 상품들의 총합 가격을 검증하지 않는다.
...
- [ ] 주문은 선택적으로 **주문 테이블의 ID**를 가질 수 있다.
  ...
  #### 주문 생성
    ...
    - 각 메뉴 항목은 **메뉴가 노출 상태**이어야 하며, 메뉴 가격이 주문 내역 아이템 가격과 일치해야 한다.
...
- [ ] 각 주문 라인 아이템(주문 내역)은 **메뉴 ID**와 연결되어 어떤 메뉴가 주문되었는지를 식별한다.
- [ ] 각 주문 라인 아이템(주문 내역)은 **주문 ID**를 참조하여 해당 주문 라인이 속한 주문을 식별한다.

...
- 메뉴(Menu)
    - 새로운 메뉴를 추가할 수 있다.
        - [X] 메뉴의 가격은 필수 값이며 음수가 될 수 없다.
        - [X] 기존에 존재하는 메뉴 그룹에 메뉴가 속해야한다.
        - [X] 메뉴는 반드시 상품을 포함해야 한다.
        - [X] 메뉴를 구성하는 상품은 이미 추가된 상품이어야 한다.
        - [X] 메뉴의 상품 수량(quantity)은 음수일 수 없다.
        - [X] 메뉴의 가격은 각 상품 가격의 총합보다 작아야한다.
        - [X] 메뉴의 이름은 필수값이며, 비어있을 수 없다.
        - [X] 메뉴의 이름은 저속해서는 안된다. (should not be profanity)
    - [X] 메뉴의 가격을 수정할 수 있다.
        - [X] 수정하는 가격은 필수값이다.
        - [X] 수정하는 가격은 음수일 수 없다.
        - [X] 수정할 메뉴는 이미 추가된 메뉴여야 한다.
        - [X] 메뉴의 가격은 각 상품가격의 총합보다 작아야한다.
    - [X] 메뉴가 보여지도록 할 수 있다.
        - [X] 보여지는 메뉴는 조회할 수 있어야 한다.
    - [X] 메뉴가 숨겨지도록 할 수 있다.
        - [X] 숨겨진 메뉴는 조회가 불가능해야 한다.
    - [X] 메뉴들의 목록을 조회할 수 있다.
- 주문 테이블(Order Table)
    - [x] 새로운 주문 테이블을 추가할 수 있다.
        - [X] 새로 추가된 테이블은 비어있으며, 0명의 손님이 앉아있다.
    - [X] 주문 테이블에 사람이 앉았음을 표시할 수 있다.
        - [X] 이미 추가된 테이블만 조작할 수 있다.
    - [X] 주문 테이블이 비게 되었음을 표시할 수 있다.
        - [X] 이미 추가된 테이블만 조작할 수 있다.
        - [X] 주문 테이블의 앉아있는 손님 숫자가 0명이 되어야 한다.
    - 주문 테이블에 앉아있는 손님의 숫자를 변경할 수 있다.
        - [X] 앉아있는 손님의 숫자는 음수가 될 수 없다.
        - [X] 이미 추가된 테이블만 손님 숫자를 변경할 수 있다.
        - [X] 비어있지 않은 테이블만 손님 숫자를 변경할 수 있다.
    - [X] 주문 테이블들의 목록을 조회할 수 있다.
- 상품(Product)
    - [X] 새로운 상품을 추가할 수 있다.
        - [X] 상품의 가격은 필수값이다.
        - [X] 상품의 가격은 음수일 수 없다.
    ...
    - [X] 상품의 가격을 수정할 수 있다.
        ...
        - [X] 상품 가격 수정으로 인해, 상품의 합한 값보다 더 비싸진 메뉴가 있다면, 메뉴가 숨겨져야 한다.
    - [X] 상품들의 목록을 조회할 수 있다.

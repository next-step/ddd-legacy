...
### 메뉴 그룹(MenuGroup)
- [ ] `1`개 이상의 `메뉴`를 어떤 분류에 의해 목록화 한 것을 `메뉴 그룹`이라고 한다.
  - 예) "추천메뉴" 메뉴그룹에는 "후라이드 치킨", "반반치킨", "순살치킨" 메뉴가 있다.
- [ ] 메뉴그룹은 이름을 등록할 수 있다.
- [ ] 가지고 있는 메뉴그룹 목록을 볼 수 있다.

### 메뉴(Menu)
- [ ] `1`개 이상의 `상품`을 구성한 것을 `메뉴`라고 부르고 고객은 등록된 메뉴를 주문한다.
  - 예) "반반치킨" 메뉴에는 "후라이드"상품, "양념"상품으로 구성되어 있다.
  - 예) 고객은 "반반치킨"메뉴를 주문한다.
- [ ] 메뉴는 이름, 가격, 메뉴노출여부, 메뉴그룹, 상품목록을 관리한다.
  - [ ] 이름은 비속어를 포함하지 않는다.
  - [ ] 메뉴는 이미 등록된 하나의 `메뉴 그룹`에 속한다.
  - [ ] 메뉴는 `메뉴구성 상품`을 `한 가지`이상 갖는다.
  - [ ] `메뉴구성 상품`은 미리 등록되어 있는 `상품`과 해당 `상품`의 수량을 `1`개 이상 가진다.
  - [ ] 가격은 `0`원 이상이다.
  - [ ] `가격 정책`은 메뉴의 가격이 {`메뉴구성 상품`들의 총 합} 보다 비싸지 않아야 한다.
    - 참고) {`메뉴구성 상품`들의 총 합} = {(해당 `상품`) * (해당 `상품`의 수량)}을 계산한 것들의 합
- [ ] 메뉴를 만들 수 있다.
- [ ] 메뉴 가격은 수정된다.
- [ ] 메뉴를 보이게 하려면 현재 메뉴의 `가격 정책`을 체크해야 한다.
- [ ] 메뉴를 보이지 않게 할 수 있다.
- [ ] 가지고 있는 메뉴 목록을 볼 수 있다.

### 상품(Product)
- [ ] 상품은 다른 것과 구성 되지 않은 단품으로 구성된 제품이다. 포스기에서 가장 작은 단위이다.
- [ ] 상품은 이름, 가격을 관리한다.
  - [ ] 이름은 비속어를 포함하지 않는다.
  - [ ] 가격은 `0`원 이상이다.
- 상품을 등록한다.
- 상품 가격은 `메뉴`의 `가격 정책`을 체크 한 후 수정한다.
- [ ] 상품 목록을 볼 수 있다.

### 주문테이블(OrderTable)
- [ ] 매장주문 고객은 테이블에 앉아 주문한다.
- [ ] 주문 테이블은 이름과 고객의 수, 사용여부를 관리한다.
  - [ ] 고객의 수는 `0`명 이상이다.
- [ ] 각 주문 테이블의 이름을 미리 정한다.
- [ ] 고객들이 테이블에 앉는다.
- [ ] 고객들이 만족스러운 식사를 마치고 식당을 나간다.
- [ ] 고객이 앉아있는 테이블의 인원을 변경한다.
- [ ] 주문 테이블 목록을 볼 수 있다.
...
- [ ] 주문은 주문종류, 주문상태, 주문시간, `주문메뉴항목`, 배달주소, `주문테이블`을 관리한다.
  ...
  - 주문상태는 "수락대기", "주문수락", "제조완료", "배달중", "배달완료", "주문종료"가 있다.
    - 아래에 [모델링 > 주문상태프로세스](#주문-상태-프로세스) 를 보면서 프로세스 이해해 봅시다.
  - [ ] 주문시간은 고객이 주문을 요청한 시간이다.
  - [ ] `주문메뉴항목`은 주문내역에 들어간 메뉴이다.
    - [ ] 주문메뉴항목은 `메뉴`, 수량, 가격를 관리한다.
    - [ ] 주문메뉴항목은 매장주문이 아니면 각 메뉴의 수량이 `1`개 이상이어야 한다.
    - [ ] 각 메뉴는 현재 구매 가능해야 한다.(노출되어 있어야 한다.)
    - [ ] **현재** 이 메뉴의 가격은 **주문 시 요청한** 주문메뉴항목의 가격과 같아야 한다.
- [ ] 주문이 들어왔습니다. (주문~ 주문~)
  - [ ] 처음 주문이 들어왔을 때 주문상태는 **수락대기중**이다.
    - [ ] **배달주문**이면 배달주소를 알아야 한다.
    - [ ] **매장주문**이면 고객은 테이블에서 주문을 해야한다.
- [ ] 주문접수를 수락합니다.
  - [ ] 수락대기 상태라면 접수를 수락합니다.
  - [ ] 배달주문일 때, 배달외부업체를 통해 배달 해줄 것을 요청합니다.
    - [ ] 요청 할 때 요청 주문 금액과 배달 주소도 보내줍니다.
- [ ] 접수 수락후 주문을 만들고 제조 완료시킵니다.
- [ ] 주문이 종료되는 과정입니다.
  - 배달 주문일 때,
    - [ ] 주문이 제조가 완료되었다면 배달을 시작합니다.
    - [ ] 배달이 완료되면 배달완료상태가 됩니다.
    - [ ] 배달완료 상태가 되면 주문은 종료됩니다.
  - 포장 주문일 때,
    - [ ] 제조가 완료되고 고객이 픽업오면 주문이 종료됩니다.
  - 매장 주문일 때,
    - [ ] 제조가 완료되면 메뉴는 서빙되어지고 해당 테이블을 완료로 만듭니다.
    - [ ] 완료상태가 되면 테이블도 비워줍니다. 
...
| 메뉴그룹    | MenuGroup     | 1개 이상의 `메뉴`를 어떤 분류에 의해 목록화 한 것              |
| 메뉴      | Menu          | 1개 이상의 `상품`을 구성한 것, 고객이 주문하는 것              |
| 비속어     | Profanity     | 공격적이거나 외설적인 말 또는 어구                         |
| 상품      | Product       | 다른 것과 구성 되지 않은 단품으로 구성된 제품이다. 가장 작은 단위이다.   |
...
| 주문테이블   | OrderTable    | 매장주문 고객이 주문을 하는 공간. 고객인원 정보도 포함한다.          |
...
| 한글명    | 영문명     | 설명                      |
|---------|---------------|-------------------------|
| 배달주문    | DELIVERY      | 주문종류 중 배달 방식, 배달외부업체 이용 |
...
| 한글명  | 영문명     |                                   |
|------|---------------|-----------------------------------|
| 수락대기 | WAITING       | 수락 대기 중. 주문을 하고 접수 수락이 되기전 까지의 상태 |
| 주문수락 | ACCEPTED      | 주문 수락. 접수된 주문을 확인하고 제조를 진행하는 상태   |
| 제조완료 | SERVED        | 제조가 완료되었다는 상태                     |
| 배달중  | DELIVERING    | 배달 주문일 때 배달이 시작되고 배달이 완료되기 까지의 상태 |

...
### 주문테이블(OrderTable)
- [ ] 매장주문 고객이 테이블에 앉아 주문하는 곳을 주문 테이블이라 한다.
- [ ] 주문 테이블은 이름과 고객의 수, 사용 여부를 관리한다.
  - [ ] 고객의 수는 `0`명 이상이다.
- [ ] 주문 테이블에 고객이 앉으면 해당 테이블을 등록한다.
  - [ ] 이름은 공백일 수 없다.
- [ ] 고객들이 테이블에 앉는다.
  - [ ] 사용여부는 사용중이 된다.
- [ ] 테이블을 초기화 한다.
...
- [ ] 주문의 제조가 완료된다.
  - [ ] 주문 접수 수락 상태에서 제조 완료상태로 바꿀 수 있다. 
- [ ] 주문이 종료된다.
  - [ ] 배달 주문일 때, 배달 중 상태 -> 배달 완료 상태를 거쳐 주문 종료상태가 된다.
    - [ ] 주문이 제조가 완료되었다면 배달을 시작한다. 주문은 배달 중 상태가 된다.
    - [ ] 배달 중 상태에서 배달이 완료되면 배달완료상태가 된다.
    - [ ] 배달완료 상태가 되면 주문은 종료 상태가 된다.
  - [ ] 포장 주문일 때 주문이 종료된다.
    - [ ] 제조 완료 상태가 되고 고객이 픽업오면 주문이 종료된다.
  - [ ] 매장 주문일 때 주문이 종료된다.
    - [ ] 제조 완료 상태에서 모든 주문 메뉴가 서빙되면 주문 종료상태가 된다.
    - [ ] 주문 테이블은 초기화 된다.
      - [ ] 테이블은 사용여부는 미사용이 된다.
      - [ ] 테이블의 고객의 수는 `0`명 이다.

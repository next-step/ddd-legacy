...
### 메뉴(Menu)
- [x] `1`개 이상의 `상품`을 구성한 것을 `메뉴`라고 부르고 고객은 등록된 메뉴를 주문한다.
  - 예) "반반치킨" 메뉴에는 "후라이드"상품, "양념"상품으로 구성되어 있다.
  - 예) 고객은 "반반치킨"메뉴를 주문한다.
- [x] 메뉴는 이름, 가격, 메뉴노출여부, 메뉴그룹, 메뉴구성 상품을 관리한다.
  - [x] 메뉴노출여부는 노출함과 노출안함 중에 선택한다.

- [x] 기능) 메뉴를 만들 수 있다.
  - [x] 이름은 필수로 입력해야 한다. 그리고 비속어를 포함하지 않는다.
  - [x] 가격은 필수로 입력해야하고 `0`원 이상이다. 그리고 `가격 정책`을 만족해야 한다.
...
- [x] 기능) 메뉴를 노출할 수 있다.
  - [x] 해당 메뉴가 미리 등록되어 있는지 체크해야한다.
  - [x] 현재 메뉴의 `가격 정책`을 체크해야 한다.
    - [x] `가격 정책`은 해당 {메뉴의 가격}이 {`메뉴구성 상품`들의 총 합} 보다 비싸지 않아야 한다.
      - 참고) {`메뉴구성 상품`들의 총 합} = {해당 `상품`의 (가격 * 수량)}을 계산한 것들의 합

- [x] 기능) 메뉴를 보이지 않게 할 수 있다.
  - [x] 해당 메뉴가 미리 등록되어 있는지 체크해야한다.
  - [x] 메뉴 노출 여부를 노출 안함으로 변경한다.

- [x] 기능) 가지고 있는 메뉴 목록을 볼 수 있다.
...
- [x] 기능) 상품을 등록한다.
  - [x] 이름은 필수로 입력해야 한다. 그리고 비속어를 포함하지 않는다.
  - [x] 가격은 필수로 입력해야 한다. 그리고 `0`원 이상이다.
...
### 주문테이블(OrderTable)
- [x] 주문 테이블은 매장주문을 위해 사용한다.
- [x] 주문 테이블은 이름, 고객의 수, 사용 여부를 관리한다.
...
- [x] 기능) 고객들이 테이블에 앉는다.
  - [x] 주문 테이블이 미리 등록되어 있는지 체크한다.
  - [X] 사용여부는 사용중이 된다.

- [x] 기능) 주문 테이블을 초기화 한다.
  - [x] 주문 테이블이 미리 등록되어 있는지 체크한다.
  - [x] `주문`의 주문상태가 **주문종료**이어야 한다.
  - [x] 사용여부는 미사용이 된다.
  - [x] 고객의 수는 `0`명이다.

- [x] 기능) 테이블의 인원을 변경한다.
  - [x] 주문 테이블이 미리 등록되어 있는지 체크한다.
  - [x] 주문 테이블은 사용여부가 사용중이어야 한다.
  - [x] 고객의 수는 `0`명 이상이다.

- [x] 기능) 주문 테이블 목록을 볼 수 있다.

### 주문(Order)
- [x] `메뉴`를 `1`개 이상 고른 후 `주문`한다.
- [x] 주문은 주문종류, 주문상태, 주문시간, `주문메뉴항목`, 배달주소, `주문테이블`을 관리한다.
  - 주문종류는 "배달", "포장", "매장"이 있다.
    - [x] 배달 : 배달 업체를 통해 주문을 받고, 메뉴들을 배달원에게 전달하면 해당 주소로 배달된다.
    - [x] 포장 : 포장을 원하는 고객이 주문하면, 고객은 메뉴들을 직접 들고간다.
    - [x] 매장 : 매장에 방문한 고객이 테이블에서 메뉴를 주문하면, 테이블에 메뉴들을 서빙한다.
  - 주문상태는 "수락대기", "주문수락", "제조완료", "배달중", "배달완료", "주문종료"가 있다.
    - 아래에 [모델링 > 주문상태프로세스](#주문-상태-프로세스) 를 보면서 프로세스 이해한다.
  - [x] 주문시간은 고객이 주문을 요청한 시간이다.
  - [x] `주문메뉴항목`은 주문내역에 들어간 메뉴이다.
    - [x] 주문메뉴항목은 `메뉴`, 수량, 가격을 관리한다.

- [x] 기능) 주문이 들어왔습니다. (주문~ 주문~)
  - [x] 주문종류는 반드시 지정해줘야 한다.
  - [x] `주문메뉴항목`은 `1`개이상 포함한다.
  - [x] `주문메뉴항목`은 `메뉴`를 중복해서 넣지 않는다.
  - [x] `주문메뉴항목`은 이미 만들어진 `메뉴`에서 선택한다.
    - [x] 주문종류가 매장주문이 아니면 해당 `메뉴`의 수량은 `0`이상이어야 한다.
    - [x] `메뉴`는 노출된 상태여야 한다.
    - [x] 각각의 **원래 `메뉴`의 가격**이 **현재 주문 시 요청한 메뉴의 가격**과 같아야 한다.
  - [x] 주문이 등록될 때는 주문상태는 **수락대기중**이다.
    - [x] **배달주문**이면 배달주소는 1자 이상이어야 한다.
    - [x] **매장주문**이면 하나의 `주문 테이블`을 선택한다.
      - [x] 미리 `주문테이블`이 등록되어 있어야한다.
      - [x] `주문 테이블`은 사용 중이어야 한다. 

- [x] 기능) 주문접수를 수락한다.
  - [x] 현재 주문 상태는 **수락대기** 상태여야 한다.
  - [x] 이미 등록되어있는 `주문`이어야 한다.
  - [ ] 주문 종료가 배달주문일 때, 배달을 요청한다.
    - [ ] {주문의 총 가격}을 계산한다.
      - [ ] {주문의 총 가격} = 모든 {`주문메뉴항목`의 (가격 * 수량)} 들의 총 합
    - [ ] 배달 요청 할 때 {주문의 총 가격}과 배달 주소를 보내준다.
  - [x] 주문 상태는 **주문수락** 상태로 변경한다.

- [x] 기능) 주문의 제조가 완료된다.
  - [x] 이미 등록되어있는 `주문`이어야 한다.
  - [x] 현재 주문 상태는 **주문수락** 상태여야 한다.
  - [x] 주문 상태는 **제조완료** 상태로 변경한다.

- [x] 기능) 배달이 시작된다.
  - [x] 이미 등록되어있는 `주문`이어야 한다.
  - [x] 주문종류는 **배달주문**이여야 한다.
  - [x] 현재 주문 상태는 **제조완료** 상태여야 한다.
  - [x] 주문 상태는 **배달중** 상태로 변경한다.

- [x] 기능) 배달이 완료된다.
  - [x] 이미 등록되어있는 `주문`이어야 한다.
  - [x] 현재 주문 상태는 **배달중** 상태여야 한다.
  - [x] 주문 상태는 **배달완료** 상태로 변경한다.

- [x] 기능) 주문이 종료된다.
  - [x] 이미 등록되어있는 `주문`이어야 한다.
  - [x] 주문종류가 **배달주문**라면
    - [x] 현재 주문 상태는 **배달완료** 상태이다.
    - [x] 주문 상태는 **주문종료** 상태로 변경한다.
  - [x] 주문종류가 **포장주문** 라면
    - [x] 현재 주문 상태는 **제조완료** 상태이다.
    - [x] 주문 상태는 **주문종료** 상태로 변경한다.
  - [x] 주문종류가 **매장주문** 라면
    - [x] 현재 주문 상태는 **제조완료** 상태이다.
    - [x] 주문 상태는 **주문종료** 상태로 변경한다.
    - [x] 주문 테이블은 초기화 된다.
      - [x] 현재 모든 `주문`의 주문 상태는 **주문종료** 상태이다.
      - [x] 테이블은 사용여부는 미사용이 된다.
      - [x] 테이블의 고객의 수는 `0`명 이다.
...
- 정리한 키친포스의 요구 사항을 토대로 테스트 코드를 작성한다.
  ...
  ####  리뷰 내용 반영
  - [x] `DEFINED_PORT` VS `RANDOM_PORT` 알아보고 `RANDOM_PORT` 적용 완료
    - `AcceptanceTestExecutionListener` 에서 LocalPort값 받아올 수 있게 세팅
    > - `DEFINED_PORT`
    >     - 테스트가 실행될 때, **애플리케이션에 이미 정의된 포트**에서 실행됩니다. (보통 8080)
    >     - 정의된 포트를 사용하므로 테스트의 반복 실행 시 **포트 충돌의 가능성**이 있습니다.
    > - `RANDOM_PORT`

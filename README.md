...
- [ ] 주문접수를 수락한다.
  - [ ] 수락대기 상태라면 접수를 수락한다.
  - [ ] 배달주문일 때, 배달외부업체를 통해 배달 해줄 것을 요청한다.
    - [ ] 요청 할 때 요청 주문 금액과 배달 주소도 보내준다.
- [ ] 접수 수락후 주문을 만들고 제조 완료된다.
- 주문이 종료되는 과정이다.
  - 배달 주문일 때,
    - [ ] 주문이 제조가 완료되었다면 배달을 시작한다.
    - [ ] 배달이 완료되면 배달완료상태가 된다.
    - [ ] 배달완료 상태가 되면 주문은 종료된다.
  - 포장 주문일 때,
    - [ ] 제조가 완료되고 고객이 픽업오면 주문이 종료된다.
  - 매장 주문일 때,
    - [ ] 제조가 완료되어 모든 주문 메뉴는 서빙되면 주문 종료상태가 된다.
    - [ ] 주문 테이블은 다른 주문을 맞이할 준비한다.

...
- [ ] 메뉴의 가격을 수정할 수 있다.
  - [ ] 가격을 변경하려는 메뉴는 반드시 존재해야 한다.
  - [ ] 메뉴의 가격은 반드시 0보다 큰 값을 가지며, 메뉴에 속한 모든 상품의 가격의 합보다 클 수 없다.
- [ ] 메뉴를 노출할 수 있다.
    - [ ] 노출하려는 메뉴는 반드시 존재해야 한다.
    - [ ] 메뉴의 속한 모든 상품의 가격의 합보다 큰 메뉴는 노출 할 수 없다.
- [ ] 메뉴를 숨길 수 있다.
    - [ ] 숨기려는 메뉴는 반드시 존재해야 한다.
- [ ] 등록된 모든 메뉴를 조회할 수 있다.
### 테이블(table)
- [ ] 테이블을 등록 할 수 있다.
  ...
  - [ ] 테이블은 반드시 이름을 가져야 한다.
- [ ] 테이블에 착석(sit) 할 수 있다.
  - [ ] 착석하려는 테이블은 반드시 존재해야 한다.
- [ ] 테이블을 정리(clear) 할 수 있다.
  - [ ] 정리하려는 테이블은 반드시 존재해야 한다.
  - [ ] 식사가 완료되지 않은 테이블은 정리 할 수 없다.
- [ ] 테이블에 손님 수를 변경 할 수 있다.
  - [ ] 인원변경할 테이블은 반드시 존재해야 한다.

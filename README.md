...
- **메뉴 (Menu)**
  ...
  - [ ] 새로운 메뉴를 등록할 수 있다.
    ...
    - [ ] 메뉴는 가격을 가진다.
      ...
      - [ ] 메뉴의 가격은 가지고있는 각각의 상품 가격을 더한 가격보다 작거나 같아야한다.
        - 메뉴의 가격 <= (A상품의 가격 * A상품의 개수) + (B상품의 가격 * B상품의 개수) ... + (N상품의 가격 * N상품의 개수) 
    - [ ] 하나의 메뉴는 하나의 메뉴 그룹에 속해야한다.
      - [ ] 메뉴가 속할 그룹은 존재하는 메뉴 그룹이어야 한다.
    - [ ] 메뉴를 등록할 때 전시 여부를 결정할 수 있다
      - [ ] 메뉴를 전시 OR 숨김 설정이 가능해야한다.
  ...
  - [ ] 메뉴는 가격을 변경할 수 있다.
    - [ ] 메뉴 가격 변경 시 가격 값은 필수값이다.
    - [ ] 메뉴 가격 변경 시 가격 값은 0이상의 양수여야한다.
      - [ ] 메뉴 가격 변경 시 가격 값이 0미만의 음수라면 사용자에게 메뉴를 등록할 수 없음을 알려야한다.
    - [ ] 가격 변경하려는 메뉴는 존재하는 메뉴여야한다.
    - [ ] 가격 변경하려는 가격은 메뉴에 포함된 상품들의 총 가격보다 작거나 같아야한다.
      - 메뉴의 가격 <= (A상품의 가격 * A상품의 개수) + (B상품의 가격 * B상품의 개수) ... + (N상품의 가격 * N상품의 개수)
      - 메뉴의 가격이 메뉴에 포함된 상품들의 총 가격보다 크다면 사용자에게 가격 변경이 불가능함을 알려야한다.
  - [ ] 메뉴는 사용자에게 전시 또는 숨김으로 설정할 수 있다.
    - [ ] 전시 상태를 변경하려는 메뉴는 존재하는 메뉴여야 한다. 
    - [ ] 메뉴를 사용자에게 전시하려고 할 때 (메뉴의 가격 > 가지고 있는 상품의 총 가격) 이라면, 메뉴는 노출시킬 수 없다.
  - [ ] 메뉴의 전체 정보를 조회할 수 있어야 한다.
    - [ ] 각 메뉴의 (메뉴고유ID, 메뉴 이름, 메뉴가격, 메뉴가 속한 그룹 정보, 메뉴 전시 여부, 메뉴 상품(메뉴에 포함된 상품)) 정보를 알 수 있어야한다.

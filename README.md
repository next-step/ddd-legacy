...
* 상품
  * 상품등록
    ...
    * [ ] 상품 가격은 null 이거나 0 보다 작을 수 없다.
    * [ ] 상품 이름은 null 이거나 비어있을 수 없다.
    * [ ] 상품 이름에는 비속어를 포함할 수 없다.
    * [ ] 상품이 정상적으로 등록되면 상품 정보를 응답한다.
  * 상품가격변경
    * [ ] 상품 가격은 상품 ID와 변경할 상품가격 정보를 입력하여 변경할 수 있다. 
    * [ ] 변경할 상품 가격은 null 이거나 0 보다 작을 수 없다.
    * [ ] 상품 ID가 null 이거나 상품이 존재하지 않으면 상품 가격을 변경할 수 없다.
...
* 메뉴그룹
  * 메뉴그룹 등록
    ...
    * [ ] 메뉴그룹은 null 이거나 비어있을 수 없다.
    * [ ] 메뉴그룹이 정상적으로 등록되면 메뉴그룹 정보를 응답한다.
  * 메뉴그룹 조회
    * [ ] 등록된 메뉴그룹 목록을 조회할 수 있다.
* 메뉴
  * 메뉴등록
    * [ ] 메뉴는 이름, 가격, 메뉴그룹ID, 진열상태, 메뉴상품들을 입력하여 등록할 수 있다.
    * [ ] 메뉴상품은 상품ID, 수량을 가지고 있다.
    * [ ] 메뉴에는 메뉴상품 여러개가 포함될 수 있다.
    * [ ] 메뉴 가격은 null 이거나 0 보다 작을 수 없다.
    * [ ] 메뉴 이름은 null 이거나 비어있을 수 없다.
    * [ ] 메뉴 이름에는 비속어를 포함할 수 없다.
    * [ ] 메뉴그룹 ID가 null 이거나 메뉴그룹이 존재하지 않으면 메뉴를 등록할 수 없다.
    * [ ] 메뉴상품 ID가 null 이거나 메뉴상품이 존재하지 않으면 메뉴를 등록할 수 없다.
    * [ ] 메뉴상품 수량이 0 보다 작을 수 없다.
    * [ ] 메뉴가격 보다 해당 메뉴에 등록된 메뉴상품금액(메뉴상품가격 * 수량)의 합이 더 높을 수 없다.
    * [ ] 상품이 정상적으로 등록되면 상품 정보를 응답한다.
  * 메뉴가격변경
    * [ ] 메뉴 가격은 메뉴ID와 변경할 메뉴가격 정보를 입력하여 변경할 수 있다.
    * [ ] 메뉴 가격은 null 이거나 0 보다 작을 수 없다.
    * [ ] 메뉴 ID가 null 이거나 메뉴가 존재하지 않으면 메뉴 가격을 변경할 수 없다.
    * [ ] 메뉴가격 보다 해당 메뉴에 등록된 메뉴상품금액(메뉴상품가격 * 수량)의 합이 더 높을 수 없다.
  * 메뉴진열
    * [ ] 메뉴는 메뉴ID를 통해 메뉴를 진열할 수 있다.
    * [ ] 메뉴 ID가 null 이거나 메뉴가 존재하지 않으면 메뉴를 진열할 수 없다.
    * [ ] 메뉴가격 보다 해당 메뉴에 등록된 메뉴상품금액(메뉴상품가격 * 수량)의 합이 더 높으면 진열할 수 없다.
  * 메뉴숨기기
    * [ ] 메뉴는 메뉴ID를 통해 메뉴를 숨길 수 있다.
    * [ ] 메뉴 ID가 null 이거나 메뉴가 존재하지 않으면 숨길 수 없다.
  * 메뉴 조회
    * [ ] 등록된 메뉴 목록을 조회할 수 있다.
* 주문
  * 주문생성
    ...
    * [ ] 주문타입은 null일 수 없다.
    * [ ] 주문상품들은 null 이거나 비어있을 수 없다.
    * [ ] 주문상품 메뉴ID가 null이거나 존재하지 않으면 주문을 생성할 수 없다.
    * [ ] 주문타입이 배달, 테이크아웃인 경우 주문상품수량이 0보다 작을 수 없다.
    * [ ] 주문상품 메뉴가 진열 중이 아니면 주문을 생성할 수 없다.
    * [ ] 주문상품 가격과 메뉴가격이 일치하지 않으면 주문을 생성할 수 없다.
    * [ ] 주문타입이 배달인 경우 배달주소가 null 이거나 비어있을 수 없다.
    * [ ] 주문타입이 매장식사인 경우 주문테이블 ID가 null이거나 존재하지 않으면 주문을 생성할 수 없다.

...
internal class OrderServiceTest {
    ...
    inner class `매장 내 식사 유형 주문 테스트` {
        ...
        inner class `주문 수락 테스트` {
            @DisplayName("주문 유형이 매장 내 식사 일 때, 주문을 수락 한다.")
            @Test
            fun test1() {
                // given
        ...
        inner class `주문 완료 테스트` {
            @DisplayName("주문 유형이 매장 내 식사인데, 주문 상태가 서빙 상태가 아니라면, IllegalStateException 예외 처리를 한다.")
            @ParameterizedTest
            @CsvSource(
                "EAT_IN, WAITING",
                "EAT_IN, ACCEPTED",
            ...
            fun test1(type: OrderType, status: OrderStatus) {
                // given
                val orderId = UUID.randomUUID()
                val order = createOrder(
                    id = orderId,
    ...
    inner class `배달 유형 주문 테스트` {
        ...
        inner class `주문 완료 테스트` {
            ...
            @DisplayName("주문 유형이 배달이고, 정상 요청이면, 주문 완료 처리가 된다.")
            @Test
            fun test2() {
                // given
                val orderId = UUID.randomUUID()
    ...
    inner class `포장 유형 주문 테스트` {
        ...
        @Nested
        inner class `주문 수락 테스트` {
            @DisplayName("주문 유형이 포장 일 때, 주문을 수락 한다.")
            @Test
            fun test1() {
                // given
                val orderId = UUID.randomUUID()
        ...
        @Nested
        inner class `주문 완료 테스트` {
            @DisplayName("주문 유형이 포장인데, 주문 상태가 서빙 상태가 아니라면, IllegalStateException 예외 처리를 한다.")
            @ParameterizedTest
            @CsvSource(
                "TAKEOUT, WAITING",
                "TAKEOUT, ACCEPTED",
            ...
            @DisplayName("주문 유형이 포장이고, 정상 요청이면, 주문 완료 처리가 된다.")
            @Test
            fun test2() {
                // given
                val orderId = UUID.randomUUID()

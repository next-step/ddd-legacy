...
class MenuServiceMockTest : BaseMockTest() {
    @Mock
    ...
    @Test
    fun `메뉴의 가격 변경 요청 - 메뉴 가격이 (메뉴상품 가격 * 메뉴상품 재고 수)의 합보다 클 경우 실패`() {
        val product = ProductSpec.of(BigDecimal.valueOf(9000))
        val menuProduct = MenuProductSpec.of(product, 2)
        val menu = MenuSpec.of(listOf(menuProduct), BigDecimal.valueOf(8000))
    ...
    @Test
    fun `메뉴의 전시상태 활성화 요청 - 정상적인 전시상태 활성화 성공`() {
        val product = ProductSpec.of(BigDecimal.valueOf(9000))
        val menuProduct = MenuProductSpec.of(product, 2)
    ...
    @Test
    fun `메뉴의 전시상태 활성화 요청 - 메뉴 가격이 (메뉴상품 가격 * 메뉴상품 재고 수)의 합보다 클 경우 실패`() {
        val product = ProductSpec.of(BigDecimal.valueOf(9000))
        val menuProduct = MenuProductSpec.of(product, 2)

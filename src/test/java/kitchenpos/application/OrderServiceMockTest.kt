...
class OrderServiceMockTest : BaseMockTest() {
    @Mock
    ...
    @Test
    fun `매장 주문 생성 - 주문 테이블이 미사용 중인 경우 실패`() {
        val orderLineItem = OrderLineItem()
        orderLineItem.menuId = menu.id
        orderLineItem.price = menu.menuProducts[0].product.price
        orderLineItem.quantity = 3
    ...
    @Test
    fun `매장 주문 생성 - 주문 테이블 미지정 후 주문 요청 시 실패`() {
        val orderLineItem = OrderLineItem()
        orderLineItem.menuId = menu.id
        orderLineItem.price = menu.menuProducts[0].product.price
        orderLineItem.quantity = 3
    ...
    @Test
    fun `매장 주문 생성 - 메뉴 가격이 주문아이템의 요청 가격과 다를 경우 실패`() {
        val orderLineItem = OrderLineItem()
        orderLineItem.menuId = menu.id
    ...
    @Test
    fun `배송 주문 생성 - 메뉴가 전시종료 상태인 경우 실패`() {
        val orderLineItem = OrderLineItem()
    ...
    @Test
    fun `배달 주문 생성 - 주문아이템의 수량이 0보다 작을 경우 실패`() {
        val orderLineItem = OrderLineItem()
        orderLineItem.menuId = menu.id
        orderLineItem.price = menu.menuProducts[0].product.price
    ...
    @Test
    fun `배달 주문 접수 - 배달 주문 상태가 WAITING이 아닐 경우 실패`() {
        val order = OrderSpec.of(
    ...
    @Test
    fun `매장 주문 전달 - 매장 주문 상태가 ACCEPTED가 아닐 경우 실패`() {
        val order = OrderSpec.of(
    ...
    @Test
    fun `매장 주문 완료 - 매장 주문 상태가 SERVED가 아닐 경우 실패`() {
        val order = OrderSpec.of(
            type = OrderType.EAT_IN,
    ...
    @Test
    fun `배송 시작 - 배달 주문의 상태가 SERVED가 아닐 경우 실패`() {
        val order = OrderSpec.of(
            type = OrderType.DELIVERY,
            status = OrderStatus.WAITING,
        )
    ...
    @Test
    fun `배송 완료 - 배달 주문의 상태가 DELIVERING이 아닐 경우 실패`() {
        val order = OrderSpec.of(
            type = OrderType.DELIVERY,
            status = OrderStatus.SERVED,
        )
    ...
    @Test
    fun `배달 주문 완료 - 배달 주문 상태 DELIVERED이 아닐 경우 실패`() {
        val order = OrderSpec.of(
            type = OrderType.DELIVERY,
            status = OrderStatus.DELIVERING,
        )

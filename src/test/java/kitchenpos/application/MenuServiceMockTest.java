...
class MenuServiceMockTest {
    ...
    private static Map<UUID, Product> 상품_목록;
    ...
    void setUp() {
        ...
        상품_목록 = Map.of(상품A.getId(), 상품A, 상품B.getId(), 상품B, 상품C.getId(), 상품C);
    ...
    void create() {
        ...
        when(productRepository.findAllByIdIn(any())).thenReturn(상품_목록.values().stream().toList());
        for (MenuProduct menuProductRequest : 메뉴A.getMenuProducts()) {
            UUID productId = menuProductRequest.getProductId();
            Product product = 상품_목록.get(productId);
    ...
    @DisplayName("메뉴 생성 시, 메뉴 가격이 음수인 경우 메뉴 생성을 실패한다")
    @Test
    void create_price_exception() {
        //given
        List<Product> products = 상품_목록.values().stream().toList();
    ...
    @DisplayName("메뉴 생성 시, 메뉴 이름이 빈 값 경우 메뉴 생성을 실패한다")
    @Test
    ...
    @DisplayName("메뉴 생성 시, 메뉴 이름이 부적절한 내용이 포함된 경우 메뉴 생성을 실패한다")
    @Test
    ...
    @DisplayName("메뉴 생성 시, 메뉴 가격이 포함된 상품 가격의 합을 초과하면 메뉴 생성을 실패한다")
    @Test
    ...
    @DisplayName("메뉴 생성 시, 메뉴 상품의 수량이 0 미만이면 메뉴 생성을 실패한다")
    @Test
    ...
    @DisplayName("메뉴 가격 변경 시, 메뉴 가격이 포함된 상품 가격의 합을 초과하면 가격 변경을 실패한다")
    @Test
    ...
    @DisplayName("메뉴 노출여부를 노출 상태로 변경한다")
    @Test
    void display() {
        //given
        List<Product> products = 상품_목록.values().stream().toList();
    ...
    @DisplayName("메뉴 노출여부를 노출 상태로 변경 시, 메뉴 가격이 포함된 상품 가격의 합을 초과하면 노출여부 변경을 실패한다")
    @Test
    ...
    @DisplayName("메뉴 노출여부를 숨김 상태로 변경한다")
    @Test
    void hide() {
        //given
        UUID menuId = UUID.randomUUID();

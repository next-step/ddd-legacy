...
class MenuBoTest {
    ...
    @DisplayName("메뉴 생성 시 가격이 없거나, 음수이면 오류 발생")
    @ParameterizedTest
    @NullSource
    ...
    @DisplayName("메뉴 생성 시 메뉴 그룹 번호가 없으면 오류 발생")
    @Test
    ...
    @DisplayName("메뉴 생성 시 메뉴 그룹에 상품이 없으면 오류 발생")
    @Test
    void createNull() {
        assertThrows(IllegalArgumentException.class, () -> menuBo.create(emptyMenuGroupMenu()));
    }

    @DisplayName("메뉴 생성 시 메뉴 그룹에 속한 상품 금액의 합이 메뉴 가격보다 작으면 오류 발생")
    @Test
    void createLessThanSum() {
        input.setPrice(BigDecimal.valueOf(8000));
